Feature: Binary Sensor Entity
  ...
  Scenario: Create with default state
    Given a binary_sensor entity "plugin-virtual.dev1.motion001" named "Motion Sensor" with on false
    When I retrieve "plugin-virtual.dev1.motion001"
  ...
  Scenario: State fields hydrate correctly including device class
    Given a binary_sensor entity "plugin-virtual.dev1.door001" named "Door Sensor" with on true and device_class "door"
    When I retrieve "plugin-virtual.dev1.door001"
    Then the binary sensor is on
    And the binary sensor device_class is "door"

  Scenario: Query by type
    Given a binary_sensor entity "plugin-virtual.dev1.motion002" named "Hall Motion" with on false
    ...
    Then the results include "plugin-virtual.dev1.motion002"
    And the results do not include "plugin-virtual.dev1.temp001"

  Scenario: Query by on state
    Given a binary_sensor entity "plugin-virtual.dev1.bsOn" named "Active" with on true
    And a binary_sensor entity "plugin-virtual.dev1.bsOff" named "Inactive" with on false
    When I query where "type" equals "binary_sensor" and "state.on" equals "true"
    Then I get 1 result

  Scenario: Delete removes entity
    Given a binary_sensor entity "plugin-virtual.dev1.bsDel" named "Sensor" with on false
    When I delete "plugin-virtual.dev1.bsDel"
    Then retrieving "plugin-virtual.dev1.bsDel" should fail
  ...
  Scenario: Raw discovery data is stored internally and hidden from queries
    Given a binary_sensor entity "plugin-virtual.dev1.motion001" named "Motion Sensor" with on false
    And I write internal data for "plugin-virtual.dev1.motion001" with payload '{"stateTopic":"zigbee2mqtt/motion/state","deviceClass":"motion"}'
    When I read internal data for "plugin-virtual.dev1.motion001"
