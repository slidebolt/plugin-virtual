...
func newBDDCtx(t *testing.T) *bddCtx {
	...
	if _, err := app.OnStart(map[string]json.RawMessage{
		...
		t.Fatalf("start plugin-virtual: %v", err)
	}
	c.app = app
...
func (c *bddCtx) iSendCommandTo(action, key string) error {
	...
	return c.sendAndWait(target, cmd)
}
...
func (c *bddCtx) iSendCoverSetPositionTo(position int, key string) error {
	...
	target := domain.EntityKey{Plugin: plug, DeviceID: dev, ID: id}
	return c.sendAndWait(target, domain.CoverSetPosition{Position: position})
}
...
func (c *bddCtx) iSendLightSetBrightnessTo(brightness int, key string) error {
	...
	target := domain.EntityKey{Plugin: plug, DeviceID: dev, ID: id}
	return c.sendAndWait(target, domain.LightSetBrightness{Brightness: brightness})
}
...
func (c *bddCtx) iSendLightSetColorTempTo(mireds int, key string) error {
	...
	target := domain.EntityKey{Plugin: plug, DeviceID: dev, ID: id}
	return c.sendAndWait(target, domain.LightSetColorTemp{Mireds: mireds})
}
...
func (c *bddCtx) iSendLightSetRGBTo(r, g, b int, key string) error {
	...
	target := domain.EntityKey{Plugin: plug, DeviceID: dev, ID: id}
	return c.sendAndWait(target, domain.LightSetRGB{R: r, G: g, B: b})
}
...
func (c *bddCtx) iSendFanSetSpeedTo(percentage int, key string) error {
	...
	target := domain.EntityKey{Plugin: plug, DeviceID: dev, ID: id}
	return c.sendAndWait(target, domain.FanSetSpeed{Percentage: percentage})
}
...
func (c *bddCtx) iSendNumberSetValueTo(value float64, key string) error {
	...
	target := domain.EntityKey{Plugin: plug, DeviceID: dev, ID: id}
	return c.sendAndWait(target, domain.NumberSetValue{Value: value})
}
...
func (c *bddCtx) iSendSelectOptionTo(option, key string) error {
	...
	target := domain.EntityKey{Plugin: plug, DeviceID: dev, ID: id}
	return c.sendAndWait(target, domain.SelectOption{Option: option})
}
...
func (c *bddCtx) iSendTextSetValueTo(value, key string) error {
	...
	target := domain.EntityKey{Plugin: plug, DeviceID: dev, ID: id}
	return c.sendAndWait(target, domain.TextSetValue{Value: value})
}

func (c *bddCtx) iSendClimateSetModeTo(mode, key string) error {
	plug, dev, id, err := parseKey(key)
	if err != nil {
		return err
	}
	target := domain.EntityKey{Plugin: plug, DeviceID: dev, ID: id}
	return c.sendAndWait(target, domain.ClimateSetMode{HVACMode: mode})
}

func (c *bddCtx) iSendClimateSetTemperatureTo(temp int, key string) error {
	plug, dev, id, err := parseKey(key)
	if err != nil {
		return err
	}
	target := domain.EntityKey{Plugin: plug, DeviceID: dev, ID: id}
	return c.sendAndWait(target, domain.ClimateSetTemperature{Temperature: float64(temp)})
}

func (c *bddCtx) sendAndWait(target domain.EntityKey, cmd messenger.Action) error {
	if err := c.cmds.Send(target, cmd); err != nil {
		return err
	}
...
func (c *bddCtx) RegisterSteps(ctx *godog.ScenarioContext) {
	...
	ctx.Step(`^I send "climate_set_mode" with hvac_mode "([^"]*)" to "([^"]*)"$`, c.iSendClimateSetModeTo)
	ctx.Step(`^I send "climate_set_temperature" with temperature (\d+) to "([^"]*)"$`, c.iSendClimateSetTemperatureTo)
